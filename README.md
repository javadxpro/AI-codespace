...
> `arena/01a07cb7-ai-codespace` است. اگر با `git clone` معمولی بگیری، بیلد
...
git clone --branch arena/01a07cb7-ai-codespace https://github.com/javadxpro/AI-codespace.git
cd AI-codespace
bash Tools/termux_build.sh          # ابزار → cmake → build → 420/420 → دستور بعدی
./build/bin/kimia_world --port 8080 --profiles build/bin/profiles

سپس در مرورگر:

- **`http://127.0.0.1:8080`** — خودِ بازی
- **`http://127.0.0.1:8080/bench`** — **KIMIA Editor**، ویرایشگر به سبک Unity
  (دکمهٔ «⚙ Editor» بالای صفحهٔ بازی هم همان‌جا می‌بردت)

در ویرایشگر می‌توانی فایل `.obj`/`.fbx` وارد کنی، فیزیک رویش بگذاری،
...
./build/bin/kimia_tests      # 420/420 tests passed
...
./build-nosdl/bin/kimia_tests    # 420/420 tests passed
```
